...
## Round 4 完整15页验证产生的待深入议题

38. **SkyRL保鲜期机制在昇腾集群的实际可用性验证**（L8-后训练专家提出）：SkyRL的"保鲜期标签+过期丢弃"机制依赖rollout从生成到进入训练的端到端延迟足够短。如果昇腾集群上这个延迟本身就超过保鲜期阈值，机制退化为"永远丢弃"。需要在Gate 1阶段实测rollout端到端延迟，确认保鲜期机制的可用性
39. **SALT有向图推算在100步以上Agent轨迹的计算扩展性**（L8-后训练专家提出）：SALT的step-level advantage需要完整轨迹回溯建图并推算，目前验证在WebShop等中等长度轨迹上。Agent任务轨迹可达100步以上，图推算的时间和内存开销能否保持可接受水平是开放问题。与parking-lot #8（CSO计算开销）属同类问题——解法本身有成本
40. **PRM并行推理作为挑战③首选PoC方向的工程评估**（L4-工程VP提出）：挑战③三个NPU能力需求中（图推算、PRM并行推理、双层模型编排），PRM并行推理与现有推理引擎能力最接近，建议作为挑战③PoC的首选切入点。需要评估：在vllm-ascend上并行运行轻量PRM的延迟和吞吐、与主模型推理的资源争用、SWE-PRM的昇腾适配工程量
41. **P12 CXL在昇腾服务器上的可行性评估**（L6-AI基础设施提出）：当前910C服务器硬件支持CXL 1.1，但CANN软件栈未适配CXL共享内存。P12作为"业界方向"展示可以，但PPT启示部分措辞需要从"NPU需要支持"改为"需要评估可行性"，避免误导为已具备能力
42. **P15收束金句对外使用的时机管控（延续）**（L1-战略VP提出）：Round 2 parking-lot #34已提出此问题。Round 4确认：收束金句"Agent时代的竞争不在谁的模型更大，而在谁的全栈更顺"内部使用OK，对外使用等Gate 2 Demo背书。正式演讲时建议在金句后加限定语"这是我们的内部判断，对外叙事需要benchmark结果支撑"

## Round 5 P3-P4 R3框架验证产生的待深入议题

43. **R3数据流在不同Agent RL框架中的映射差异验证**（L8-后训练专家提出）：R3（Routing-Record-Reply）作为叙事模型已通过验证，但不同框架的实际数据流有差异——verl的同步rollout中Routing和Record是耦合的，ProRL Agent的异步架构中三步更接近串行。需要确认R3模型对所有目标框架的适用性，避免演讲后被追问"你说的R3在XX框架里不是这么走的"
44. **C编号体系正式定义文档**（L4-工程VP提出）：6+1子挑战编号（C1策略时效性、C2采集吞吐、C3推理效率、C4奖励与归因、C5数值可信、C6环境保真、C7工程可达）在洞察汇报中降为背景索引，但在后续技术专题和Gate check项中需要正式定义。建议出一份1页的子挑战定义表，含编号、名称、归属栈层、对应R3环节、Gate check映射
