...
### ① Agent并发rollout的显存墙 — P3挑战❶方面b

**Agent场景为什么更严重**：普通LLM推理是"一个请求一次生成"，显存压力可控。但Agent RL训练需要**千条rollout同时并行生成**，每条rollout还是多轮交互——KV Cache随工具调用轮次持续膨胀（单Agent会话可达数GB）。千并发 × 多轮膨胀 = 显存需求是普通serving的10-100倍。70B模型全精度推理需约140GB，不量化则并发度被压到个位数，训练数据产出速率崩塌。

**关键数据**：QeRL（NVIDIA，ICLR'26）靠NVFP4量化首次在单张H100 80GB上实现32B模型的RL训练——量化在Agent RL中不是"优化"而是"前提条件"。

### ② 量化偏差被RL训练循环指数放大 — P3挑战❻方面a

**Agent场景为什么更严重**：在普通推理serving中，量化损失2-3%精度，用户几乎无感。但在RL训练中，量化引入的logprob偏差会被**importance ratio放大机制**逐步累积——PPO的ratio = π_new/π_old，分子分母各有量化偏差，ratio本身就带有系统性bias。Agent RL的Episode长达数十到数百步，每步的偏差通过RL的"策略→数据→梯度→策略"闭环不断自我强化，几百步后可导致训练发散。这在单轮SFT/DPO中完全不存在。

**关键数据**：MXFP4与NVFP4之间约10%精度差距，在LLM推理中可接受，但在RL训练中通过ratio放大机制会导致训练崩溃。MBS+OAS将差距压到<1%才让MXFP4在RL中可用（Meta, arXiv:2603.08713）。

### ③ Agent Episode分钟级放大rollout速度瓶颈 — P3挑战❶方面b

**Agent场景为什么更严重**：传统LLM RL（如数学推理）的一次rollout生成几百token，秒级完成。但Agent RL的一个Episode涉及多轮工具调用（代码编译、浏览器交互、API调用），每轮都要等环境返回结果，单条轨迹耗时**分钟级**。rollout速度 = 训练数据供给速率，推理慢一倍→训练就慢一倍。而且Agent轨迹的token量是数学推理的10-100倍（几千到几万token），每步decode要读取的KV Cache也更大。不加速则训练GPU持续空等数据。

**关键数据**：HuggingFace调研显示rollout推理占RL训练80-90%运行时；Agent场景因环境等待进一步膨胀到90%+。Eagle3投机解码+4bit量化的组合可将rollout吞吐提升3-5倍。
...
├──────┬─────────────────┬─────────────────┬───────────────────────────┤
│      │ ① 千并发rollout    │ ② 量化偏差被RL    │ ③ Episode分钟级          │
│ 核心 │   显存墙          │   训练循环放大     │   放大速度瓶颈           │
│ 挑战 │ 千条×多轮×数GB/条 │ ratio逐步累积偏差 │ 推理慢1x=训练慢1x       │
│      │ =serving的10-100x │ 几百步后训练发散   │ Agent轨迹=推理10-100x   │
├──────┼─────────────────┼─────────────────┼───────────────────────────┤
│      │                 │                 │                           │
