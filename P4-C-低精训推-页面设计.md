...
### ③ Rollout生成速度瓶颈 — P3挑战❶方面b
Agent RL训练中90%算力消耗在rollout推理上，推理生成速度直接等于训练数据供给速率。Agent的Episode动辄分钟级，传统自回归逐token解码是memory-bandwidth的囚徒。不加速则训练GPU持续等待数据，利用率不到30%。

**关键数据**：HuggingFace调研显示rollout推理占RL训练80-90%运行时；Eagle3投机解码实现2-3x decode加速（Google/AWS/NVIDIA生产部署）。
...
├──────┬─────────────────┬─────────────────┬───────────────────────────┤
│      │ ① 推理显存瓶颈    │ ② 量化精度损失放大│ ③ rollout生成速度         │
│ 核心 │ 70B BF16=140GB   │ MXFP4 vs NVFP4  │ 90%算力在推理             │
│ 挑战 │ 单卡放不下        │ 精度差距~10%     │ 逐token串行=bandwidth囚徒 │
│      │ rollout并发受限   │ ratio放大偏差    │ 不加速则数据饥饿          │
├──────┼─────────────────┼─────────────────┼───────────────────────────┤
│      │                 │                 │                           │
